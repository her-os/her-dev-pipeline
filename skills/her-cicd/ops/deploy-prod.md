...
> **🗄️ 本文档已归档（2026-06-12）**：生产部署 = 合 main 后 K8s 自动发布，见 `context/k8s-deploy-pipeline.md`。**不要再走本文档的 release.sh 流程**，除非以下两种情况：
> ① DNS 尚未切换到 K8s 的过渡期内需要发生产（切换状态见 k8s-deploy-pipeline.md 顶部）；
> ② K8s 故障、DNS 已切回 CVM 应急轨道时。
>
