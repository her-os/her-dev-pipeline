...
| 入口 | 作用 |
|------|------|
| `https://test.hersoul.cn` | her-web 测试版（**首选入口**，AUTH_URL/app_url 即此域名，Let's Encrypt 自动证书） |
| `https://test.hersoul.cn/test-gateway` | gateway 测试版（Traefik stripPrefix） |
| `http://192.144.187.174:80` | her-web 测试版（IP 直连备用，浏览器登录可用——已加入 AUTH_TRUSTED_ORIGINS） |
| `http://192.144.187.174:80/test-gateway` | gateway 测试版（IP 直连备用） |
| ~~`http://www.roome.cn`~~ | 已废弃：域名未备案，证书申请被 DNSPod 拦截 |
...
`refresh-web-db` 安全修正：清空 session、改 app_url 为 `https://test.hersoul.cn`、改支付回调为 test 域名、应用兼容 schema。
...
### test.hersoul.cn 浏览器登录报「操作失败」（已修复 2026-06-11）

历史原因：`AUTH_URL`/`NEXT_PUBLIC_APP_URL` 曾写死 IP，better-auth origin 校验拒绝域名来源。已修：deploy-test.sh 的 `WEB_URL` 默认 `https://test.hersoul.cn`，IP 入口通过 `AUTH_TRUSTED_ORIGINS=http://192.144.187.174` 保留浏览器登录能力。若复现，检查 `her-web-test` env 这三个变量是否齐全（`AUTH_URL` / `NEXT_PUBLIC_APP_URL` 为域名 + `AUTH_TRUSTED_ORIGINS` 含 IP）。注意 curl 无 Origin 头时永远不报错，验证必须带 `Origin` 头：origin 拒绝返回 403，通过返回 401（密码错）。
