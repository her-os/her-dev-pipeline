...
| 17 | 监控 grep 容器日志必须滤 SQL 行+截断超长行 | GORM 慢查询会把含完整请求体（可达 MB 级）的 SQL 打进 docker logs，列名 `read_error` 含 error 字样被关键词误中，且每条内容不同令去重指纹失效 → 2026-06-12 Hermes 一天发 119 条告警、单条被飞书切片成几十条。日志监控管道必须：滤 `INSERT INTO/SELECT` 行、去 ANSI 码、单行截断、指纹剔除数字。详见 `ops/hermes-monitor.md` |
...
## changelog 与 Hermes 双向同步（坚果云）

changelog.md 真身在 `~/Documents/夙愿's库/her-ops-sync/changelog.md`（坚果云同步对），skill 目录里是软链接；VPS Hermes 的 `~/.hermes/skills/her-ops/changelog.md` 同样软链接到它的同步副本。**两边写入约 10 秒互通**。

- Hermes（有 gateway Admin API 写权限）每次变更会追加 `[Hermes]` 条目——任务涉及 gateway 渠道/quota/监控/云资源时，**动手前看 changelog 尾部有没有新的 `[Hermes]` 条目**（直接读本地文件即可，无需 ssh）
- 本地收尾照常写 changelog，Hermes 自动看到，无需额外动作
- 两边同时写会产生坚果云冲突副本（`坚果云冲突文件夹`）——发现条目丢失时去那里找回手工合并
